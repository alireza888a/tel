  ...
  return (
    <GlassCard title="تنظیمات نظرسنجی و آزمون‌ساز"className="border-t-4 border-t-emerald-500">
      <div className="space-y-6">
        <div>
          <label className="text-sm text-slate-500 mb-2 block">پرسش یا سوال شما</label>
          <input
            value={pollQuestion}
            onChange={e => setPollQuestion(e.target.value)}
            placeholder="سوال خود را مطرح کنید (مثال: برنده بازی امشب کیست؟)"
            className="w-full bg-slate-100 border border-slate-200 rounded-xl p-3 text-slate-800 outline-none focus:border-emerald-500"
          />
        </div>

        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="text-sm text-slate-500 mb-2 block">نوع نظرسنجی</label>
            <select
              value={pollType}
              onChange={e => {
          ...
          <div>
            <label className="text-sm text-slate-500 mb-2 block">حالت رای‌دهی</label>
            <select
        ...
        {pollType ==='regular'&& (
          <label className="flex items-center gap-3 bg-slate-100 p-3 rounded-xl border border-slate-100 cursor-pointer">
            <input
              type="checkbox"
              checked={allowMultipleAnswers}
              onChange={e => setAllowMultipleAnswers(e.target.checked)}
              className="rounded bg-slate-100 border-slate-200 text-emerald-600 focus:ring-0"
            />
            <span className="text-xs text-slate-600">امکان انتخاب چند گزینه همزمان</span>
          </label>
        )}

        <div className="space-y-3">
          <div className="flex justify-between items-center">
            <label className="text-sm text-slate-500 font-bold">گزینه‌های پاسخ (حداقل ۲ و حداکثر ۱۰ گزینه)</label>
            {pollOptions.length < 10 && (
              <button
                onClick={handleAddPollOption}
                className="text-xs text-emerald-600 bg-emerald-500/10 hover:bg-emerald-500/20 px-2.5 py-1 rounded-lg border border-emerald-500/20 flex items-center gap-1 transition-colors"
              >
            ...
            {pollOptions.map((opt, idx) => (
              <div key={idx} className="flex gap-2 items-center">
                ...
                <input
                  ...
                  className="flex-1 bg-slate-100 border border-slate-200 rounded-lg p-2.5 text-xs text-slate-800 outline-none focus:border-emerald-500"
                />
                {pollType ==='quiz'&& (
                  <button
                    ...
                    className={`px-3 py-2 rounded-lg text-[10px] font-bold border transition-all shrink-0 ${
                      quizCorrectOptionIndex === idx
                        ?'bg-green-600 border-green-500 text-slate-800'
                ...
                {pollOptions.length > 2 && (
                  <button
                    ...
                    className="p-2 text-red-600 hover:bg-red-500/10 rounded-lg shrink-0"
                  >
                    <Trash2 size={14}/>
                  </button>
                )}
        ...
        {pollType ==='quiz'&& (
          <div className="space-y-2 pt-2 border-t border-slate-100">
            <label className="text-xs text-slate-500">توضیح یا راهنمایی پاسخ (اختیاری)</label>
            <textarea
              value={quizExplanation}
              onChange={e => setQuizExplanation(e.target.value)}
              placeholder="توضیح دهید چرا این گزینه صحیح است. پس از کلیک روی گزینه اشتباه توسط کاربر، نمایش داده می‌شود."
              className="w-full h-20 bg-slate-100 border border-slate-200 rounded-xl p-3 text-xs text-slate-800 resize-none outline-none focus:border-emerald-500"
            />
          </div>
        )}
      </div>
