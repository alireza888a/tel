  ...
  return (
    <div className="absolute inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-fade-in">
      <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-2xl shadow-2xl flex flex-col max-h-[90vh]">
        {/* Header */}
        <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-100 rounded-t-2xl">
          <div>
            <h3 className="text-xl font-bold text-slate-800 flex items-center gap-2">
              <ListChecks size={24} className="text-green-600"/> طراحی سوالات فرم
            </h3>
            <p className="text-xs text-slate-500 mt-1">
              شناسه فرم: <span className="font-mono text-slate-500">{editingFormId}</span>
            </p>
          </div>
          <button
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar flex-1 space-y-6">
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-4">
            <div>
              <label className="text-sm text-slate-600 font-bold mb-1.5 block">
                عنوان فرم
              </label>
              <input
                type="text"
                ...
                className="w-full bg-slate-100 border border-slate-200 rounded-lg p-2.5 text-slate-800 text-sm outline-none focus:border-blue-500"
              />
            </div>
            ...
            <div className="pt-2 border-t border-slate-100">
              <label className="flex items-center gap-3 cursor-pointer">
                ...
                />
                <span className="text-sm font-medium text-slate-800">نمایش در Mini App</span>
              </label>
              <p className="text-[11px] text-slate-500 mt-1.5 leading-relaxed bg-slate-100 p-2.5 rounded-lg border border-slate-100">
          ...
          <div className="bg-blue-500/5 border border-blue-500/20 rounded-xl p-4">
            <label className="text-sm text-blue-600 font-bold mb-2 block flex items-center gap-2">
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-end">
              <label className="text-sm text-slate-500 font-bold">لیست سوالات</label>
              <span className="text-xs text-slate-500">{currentForm.questions?.length || 0} سوال</span>
            </div>

            {(currentForm.questions || []).map((q, idx) => (
              <div key={q.id} className="group flex flex-col md:flex-row items-start gap-3 bg-slate-100 border border-slate-100 rounded-xl p-3 hover:border-slate-300 transition-all">
                <div className="w-6 h-6 rounded-full bg-slate-200 flex items-center justify-center text-xs font-bold text-slate-500 mt-2 shrink-0">
                  {idx + 1}
                </div>
                ...
                <div className="flex-1 w-full space-y-2">
                  <textarea
                    ...
                    className="w-full bg-transparent border-b border-slate-200 outline-none text-slate-800 text-sm resize-none h-10 py-1"
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <select
                      ...
                      className="bg-slate-100 text-xs text-slate-800 border border-slate-200 rounded p-1 outline-none"
                    >
                  ...
                  {(q.type ==='select'|| q.type ==='checkbox') && (
                    <div className="mt-3 p-3 bg-slate-100 border border-slate-200 rounded-xl space-y-2">
                      ...
                      <div className="space-y-1.5">
                        {(q.options || []).map((opt, optIdx) => (
                          <div key={optIdx} className="flex items-center gap-2 bg-slate-100 px-2.5 py-1.5 rounded-lg border border-slate-100 text-xs text-slate-800">
                      ...
                      <div className="flex gap-2 pt-1">
                        <input
                          ...
                          className="flex-1 bg-slate-100 border border-slate-200 rounded-lg px-2.5 py-1.5 text-xs text-slate-800 outline-none focus:border-blue-500"
                          onKeyDown={(e) => {
                            if (e.key ==='Enter') {
                              e.preventDefault();
                        ...
                        <button
                          ...
                          className="bg-blue-600 hover:bg-blue-500 text-slate-800 text-xs px-3 py-1.5 rounded-lg font-medium transition-colors shrink-0"
                        >
                          افزودن
                        </button>
                      </div>
                ...
                <button
                  ...
                  className="p-2 text-red-600 hover:bg-red-500/10 rounded-lg opacity-0 group-hover:opacity-100 transition-all self-start md:self-center"
                  title="حذف سوال"
                >
                  <Trash2 size={16} />
                </button>
            ...
            <button
              ...
              className="w-full py-3 border-2 border-dashed border-slate-200 hover:border-white/30 rounded-xl text-slate-500 hover:text-slate-900 transition-all flex items-center justify-center gap-2 text-sm font-bold"
            >
              <Plus size={16} />
              افزودن سوال جدید
            </button>
        ...
        <div className="p-4 border-t border-slate-100 bg-slate-100 rounded-b-2xl flex justify-end">
          <button
            onClick={() => setEditingFormId(null)}
            className="px-6 py-2 bg-green-600 hover:bg-green-500 text-slate-800 rounded-xl shadow-lg shadow-green-600/20 transition-all font-bold flex items-center gap-2"
          >
            <Check size={18} />
            ذخیره و بستن
          </button>
