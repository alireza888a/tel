    ...
    <div className="space-y-4 animate-fade-in max-w-lg mx-auto">
      <div className="bg-white border border-slate-100 shadow-sm rounded-2xl p-5 space-y-4">
        <div className="flex items-center gap-3 border-b border-slate-50 pb-3">
          ...
          <div>
            <h2 className="text-sm font-bold text-slate-800">پشتیبانی و ارتباط با مدیریت</h2>
            <p className="text-[11px] text-slate-500">سوال یا مشکل خود را مطرح کنید تا پاسخ داده شود.</p>
          </div>
        </div>

        {supportSuccess && (
          <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs leading-relaxed flex items-start gap-2 animate-fade-in">
        ...
        <form onSubmit={handleSupportSubmit} className="space-y-3">
          <div>
            <label className="block text-xs font-medium text-slate-600 mb-1.5">متن پیام یا درخواست شما</label>
            <textarea
              rows={5}
              value={supportText}
              onChange={(e) => setSupportText(e.target.value)}
              placeholder="پیام خود را بنویسید..."
              className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs text-slate-800 placeholder-slate-400 outline-none focus:border-blue-400 transition-colors resize-none leading-relaxed"
              required
            />
          </div>
