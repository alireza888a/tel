    ...
    <GlassCard className="border-t-4 border-t-purple-500">
      <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-bold text-lg text-brand-navy">مدیریت گالری تصاویر (Mini App)</h3>
        </div>
      ...
      {galleryImages.length === 0 ? (
        <div className="text-center py-8 border-2 border-dashed border-black/10 rounded-2xl bg-black/[0.03] text-brand-navy/50 text-xs">
          هنوز تصویری به گالری اضافه نشده است. روی «افزودن عکس جدید» کلیک کنید.
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
          {galleryImages.map((img) => (
            <div key={img.id} className="bg-black/[0.03] border border-black/10 rounded-xl p-3 flex flex-col gap-2.5">
              <div className="relative w-full h-36 rounded-lg bg-black/[0.05] overflow-hidden border border-black/5 flex items-center justify-center">
                {getDisplayableImageUrl(img.imageUrl) ? (
                  ...
                  <div className="flex flex-col items-center justify-center text-brand-navy/40 gap-1">
                    <ImageIcon size={24} />
                    <span className="text-[10px] text-brand-navy/50">تصویر غیرقابل نمایش</span>
                  </div>
                )}
                <button
              ...
              <input
                ...
                className="w-full bg-white border border-black/10 rounded-lg p-2 text-xs text-brand-navy placeholder-brand-navy/30 outline-none focus:border-purple-500"
              />
            </div>
          ))}
        </div>
