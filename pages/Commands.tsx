        ...
        <div className="max-w-7xl mx-auto space-y-6 animate-fade-in pb-10">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1.5fr_1fr] gap-6">
                ...
                <div className="space-y-4">
                    {commands.length === 0 && (
                         <div className="bg-black/[0.03] border-2 border-dashed border-black/10 rounded-xl p-8 text-center flex flex-col items-center gap-3">
                             <Command size={48} className="text-brand-navy/20"/>
                             <p className="text-brand-navy/50">هنوز دستوری تعریف نکرده‌اید.</p>
                             <button onClick={addCommand} className="text-blue-600 text-sm hover:underline">افزودن اولین دستور</button>
                         </div>
                    )}
                    ...
                    {commands.map((cmd, index) => (
                        <GlassCard key={index} className="relative group overflow-visible">
                             <button 
                                ...
                                className="absolute top-2 left-2 text-red-400 hover:text-red-600 p-1 transition-colors"
                             >
                                 <Trash2 size={16}/>
                             </button>
                             ...
                             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                 ...
                                 <div className="space-y-3">
                                     <div>
                                         <label className="text-xs text-brand-navy/50 block mb-1">نام دستور (بدون /)</label>
                                         <div className="relative">
                                             <span className="absolute top-1/2 -translate-y-1/2 right-3 text-brand-navy/40 text-sm">/</span>
                                             <input 
                                                 value={cmd.command}
                                                 onChange={(e) => updateCommand(index,'command', e.target.value.toLowerCase())}
                                                 placeholder="start"
                                                 className="w-full bg-black/[0.03] border border-black/10 rounded-lg py-2 pr-6 pl-2 text-brand-navy outline-none dir-ltr text-left font-mono focus:border-orange-500"
                                                 dir="ltr"
                                             />
                                         </div>
                                     </div>
                                     <div>
                                         <label className="text-xs text-brand-navy/50 block mb-1">توضیحات (نمایش در لیست)</label>
                                         <input 
                                             value={cmd.description}
                                             onChange={(e) => updateCommand(index,'description', e.target.value)}
                                             placeholder="شروع مجدد ربات"
                                             className="w-full bg-black/[0.03] border border-black/10 rounded-lg p-2 text-brand-navy outline-none focus:border-blue-500"
                                         />
                                     </div>
                                 </div>

                                 {/* Right: Logic */}
                                 <div className="bg-black/[0.03] rounded-lg p-3 border border-black/5">
                                     <div className="mb-2 flex items-center gap-2 text-xs text-brand-navy/60">
                                         <Play size={12}/>
                                         عملیات اجرایی:
                                     </div>
                                     ...
                                     <div className="flex gap-2 mb-3">
                                         <button 
                                            ...
                                            className={`flex-1 py-1.5 text-xs rounded-md border ${cmd.actionType ==='menu'?'bg-purple-600 border-purple-500 text-white':'border-black/10 text-brand-navy/50 hover:bg-black/[0.03]'}`}
                                         >
                                             باز کردن منو
                                         </button>
                                         <button 
                                            onClick={() => updateCommand(index,'actionType','text')}
                                            className={`flex-1 py-1.5 text-xs rounded-md border ${cmd.actionType ==='text'?'bg-green-600 border-green-500 text-white':'border-black/10 text-brand-navy/50 hover:bg-black/[0.03]'}`}
                                         >
                                             ارسال متن
                                         </button>
                                     </div>
                                     ...
                                     {cmd.actionType ==='menu'? (
                                         <select 
                                            ...
                                            className="w-full bg-black/[0.03] border border-black/10 rounded p-2 text-sm text-brand-navy outline-none"
                                         >
                                         ...
                                         <textarea 
                                             ...
                                             className="w-full h-20 bg-black/[0.03] border border-black/10 rounded p-2 text-xs text-brand-navy outline-none resize-none"
                                         />
                                     )}
                                 </div>
                             </div>
                    ...
                    <button 
                        onClick={addCommand}
                        className="w-full py-3 border-2 border-dashed border-black/10 hover:border-black/20 rounded-xl text-brand-navy/50 hover:text-brand-navy transition-all flex items-center justify-center gap-2 text-sm font-bold"
                    >
                        <Plus size={16} />
                        افزودن دستور جدید
                    </button>
                    ...
                    <GlassCard title="راهنمای دستورات ویژه">
                         <div className="space-y-3 text-sm text-brand-navy/70">
                             <div className="flex items-start gap-2">
                                 <span className="font-mono bg-orange-500/10 px-1 rounded text-orange-700">/start</span>
                                 <p className="text-xs">معمولاً برای"شروع"استفاده می‌شود. بهتر است آن را به منوی <span className="text-brand-navy font-bold">root</span> متصل کنید.</p>
                             </div>
                             <div className="flex items-start gap-2">
                                 <span className="font-mono bg-orange-500/10 px-1 rounded text-orange-700">/clear</span>
                             ...
                             <div className="flex items-start gap-2">
                                 <span className="font-mono bg-orange-500/10 px-1 rounded text-orange-700">/help</span>
